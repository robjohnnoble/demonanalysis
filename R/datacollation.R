...
#' @importFrom stats var
#' @export
get_Variable_cor_summary <- function(summary,MainVariable,  col_names_list, num_parameters, min_count, Verbose=FALSE,ReturnCI=FALSE) {
  
  col_nums <- c(1:num_parameters, which(colnames(summary) == "start_size"))
  
  col_nums <- col_nums[ ! col_nums %in% which(colnames(summary) %in% c("seed") )]
