...
#' 
#' @export
plot_corr_waiting_time_versus_depth <- function(df, output_filename = NA, file_type = "png", output_dir = NA) {
  
  par(mfrow=c(1,1))
  par(mar=c(4.5,5,1,1))
  start_size <- 2500
  col = rainbow(10)[1]
  sizes <- c(6500, 7500, 11500, 20500)
  start_size <- sizes[1]
  cols1 <- c("red", "orange", "green3", "blue")
  col = cols1[1]
  par(oma = c(0, 0, 0, 3))
  plot(Cor_Driver_Diversity ~ depth, data = df, type = "l", ylim = c(-1, 0), col = col,
       xlab = "measurement distance from periphery (%)", ylab = "correlation coefficient: diversity vs waiting time", xaxt = "n", lwd = 2)
  axis(1, at=2*0:5,labels=20*0:5)
  for(i in 2:4) {
    start_size <- sizes[i]
    col = cols1[i]
    lines(Cor_Driver_Diversity ~ depth, data = df, col = col, lwd = 2)
  }
  par(fig = c(0, 1, 0, 1), oma = c(0, 0, 0, 0), mar = c(0, 0, 0, 0), new = TRUE)
  plot(0, 0, type = "n", bty = "n", xaxt = "n", yaxt = "n")
  legend("bottomright", as.character(sizes), title = "size at measurement", xpd = TRUE, 
         horiz = FALSE, inset = c(0.2, 0.3), bty = "n", lty=1, col = cols1[1:4], lwd = 2)
}
